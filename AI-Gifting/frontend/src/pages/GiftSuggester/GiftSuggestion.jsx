...
//import ProductSearch from './Product.jsx';
...
const handleGenerateSuggestions = async () => {
  try {
      ...
      body: JSON.stringify({
        prompt: `Suggest gifts for a ${age}-year-old ${relationship} for ${occasion} who likes ${interests} with budget of Rs. ${budget} .`,
        max_tokens: 300, 
      }),
    });
    if (!response.ok) {
    ...
    if (response.ok) {
      ...
      const generatedText = data.suggestions;
      const suggestionArray = generatedText.split('\n');
      setSuggestions(suggestionArray);
      //setSuggestions(generatedText);
      //const responseData = data.choices[0].text; // Assuming the response contains a single suggestion
      ...
      <div className='details'>
        <form className='form'>
          ...
          <hr className="my-4"></hr>
            <button  onClick={handleGenerateSuggestions} className="btn-4" type="submit">Get gift suggestions</button>
          <hr className="my-4"></hr>
          <button onClick={()=>navigate('/products')} className="Btn" type="submit">Get Products</button>
        </form>
        </div>
      </div>
